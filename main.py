...
# df['city'].fillna(np.min(df['city']), inplace=True)
...
# except KeyError:
# st.warning("Column 'New deaths' not found in the dataset.")
...
st.pyplot(plt)
# st.subheader("English grade")
# plt.figure(figsize=[10, 10])
# plt.pie(df["english.grade"].head(60).value_counts().values,
#                 autopct='%1.1f%%',
#                 labels=df["english.grade"].head(60).value_counts().index,
#                 colors=['#ff9999', '#66b3ff', '#99ff99', '#ffcc99'])
# st.pyplot(plt)
# st.subheader("Science grade")
# plt.figure(figsize=[10, 10])
# plt.pie(df["sciences.grade"].head(60).value_counts().values,
#                 autopct='%1.1f%%',
#                 labels=df["sci`ences.grade"].head(60).value_counts().index,
#                 colors=['#ff9999', '#66b3ff', '#99ff99', '#ffcc99'])
# st.pyplot(plt)
# st.subheader("Language grade")
# plt.figure(figsize=[10, 10])
# plt.pie(df["language.grade"].head(60).value_counts().values,
#                 autopct='%1.1f%%',
#                 labels=df["language.grade"].head(60).value_counts().index,
#                 colors=['#ff9999', '#66b3ff', '#99ff99', '#ffcc99'])
# st.pyplot(plt)
